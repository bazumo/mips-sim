 ...
 */
 export default function(architecture) {
   return class InstructionToken extends ParserToken {
          ...
          let newres = r.parser.parseAndMap(
            SequentialToken(
              "",
              {
                ...
                instruction: ExactlyOneToken(InstructionParametersToken(architecture, r.instruction))
              }
            ),
            (a) => {
              nres.push(
    ...
    constructor(parser, sourceStart, sourceLine, sourcePosInLine, instructionNameToken, parametersToken) {
    ...
    getAssembledLength() {
      let r = this.instruction.getAssembledLength(architecture, this.parameters);
      if (r < 0) return super.getAssembledLength();
      return r;
    }

    writeAssembly(dataView, index) {
      return this.instruction.writeAssembly(architecture, this.parameters, dataView, index);
    }
  }
}
